...
class DiffPoolBatchedGraphLayer(nn.Module):
    ...
    def __init__(self, input_dim, assign_dim, output_feat_dim,
                 activation=F.relu, dropout=0, aggregator_type='meanpool', link_pred='False'):
    ...
    def forward(self, g, h):
        ...
        graph.add_nodes(h.shape[0])
        graph.ndata['feat'] = h
        edge_list = np.nonzero(adj_new)
        graph.add_edges(edge_list[0], edge_list[1])
