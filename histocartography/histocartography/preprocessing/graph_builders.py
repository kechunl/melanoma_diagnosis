...
class KNNGraphBuilder(BaseGraphBuilder):
    ...
    def _build_topology(
        ...
        try:
            ...
            edge_list_x = []
            edge_list_y = []
            for centroid in tqdm(centroids, total=len(centroids)):
                dist = np.sqrt(np.sum(np.square(centroids - centroid), axis=1))
                if self.thresh is not None:
                    dist[dist > self.thresh] = 0
                edge_list = np.nonzero(dist[None, :])
                edge_list_x.extend(edge_list[0])
                edge_list_y.extend(edge_list[1])
            graph.add_edges(edge_list_x, edge_list_y)
            return
